...
bedrock.events.on('bedrock-views.cli.optimize.init', (command) => {
  command
    ...
    .option('--webpack-uglify-mangle <mode>',
      'webpack uglifyjs mangle mode (true, false) [false]',
      /^(true|false)$/i, 'true')
...
api.optimize = function(options, callback) {
  ...
  logger.info(
    `[bedrock-views] webpack optimizing: "${entry}" to: "${output}"`);
  ...
  const webpackConfig = webpackMerge(
    baseConfig, ...config['bedrock-webpack'].configs);
  ...
  webpack(webpackConfig, (err, stats) => {
    ...
    if(stats.hasWarnings()) {
      logger.warn('[bedrock-webpack] webpack warnings', info.warnings);
    }

    // FIMXE: log important parts of stats/info
    //logger.info('[bedrock-views] webpack stats', stats.xxxxxx);
