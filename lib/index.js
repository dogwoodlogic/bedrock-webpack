...
function _configureWatchCli(command) {
  return command
...
// configure for the "watch" option
bedrock.events.on('bedrock-views.cli.watch.configure',
  command => _configureWatchCli(command));
...
api.optimize = callbackify(async function(options = {}) {
  ...
  // FIXME
  const isOptCmd = bedrock.config.cli.command.name() === 'optimize';

  // run watch in non-minify mode or if requested
  if(!isOptCmd &&
    (bedrock.program.minify === 'false' || bedrock.program.watch === 'true')) {
    logger.info('watch starting');
    const compiler = webpack(webpackConfig);
    // FIXME: use compiler hooks for status logging
    compiler.watch({}, (err, stats) => {
      webpackDone(err, stats);
      const dt = stats.endTime - stats.startTime;
      logger.info('watch build done', {timeMs: dt});
      // FIXME: handle errors?
    });
    // resolve immediately
    return;
