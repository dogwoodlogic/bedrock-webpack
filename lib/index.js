    ...
    if(hmr) {
      bedrock.events.on('bedrock-express.configure.router', app => {
        ...
        app.get(path, (req, res, next) => {
          ...
          // which point it serializes any state (including session state) to disk,
          // breaking the Web app in a variety of ways; overriding `res.end`
          // stops this bad behavior
          res.end = () => {};
...
async function _buildOverrideConfigs({pkgs = {}}) {
  ...
  const configs = [];
  const sysPkgs = bedrock.config.views.bundle.packages.map(async pkg => {
