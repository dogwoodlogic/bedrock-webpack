...
function _configureBundleCli(command) {
  return command
    ...
    .option('--webpack-js-comments <mode>',
      ...
      ' (true, all, some, false, default) [default]',
...
async function _buildOverrideConfigs({pkgs = {}}) {
  ...
  const configs = [];
  const sysPkgs = bedrock.config.views.system.packages.map(async pkg => {
