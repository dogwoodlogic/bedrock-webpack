...
function _buildAliases(callback) {
  async.mapSeries(bedrock.config.views.system.packages, (pkg, callback) => {
    fs.readFile(pkg.manifest, (err, data) => {
      ...
      // get the custom webpack config
      const man = (manifest.bedrock || {}).webpack;

      // iterate over the keys of custom webpack config which correspond
      // to a package name
      Object.keys(man || {}).forEach(pkgName => {
        // only support adding extra aliases
        if((man[pkgName] || {}).resolve) {
          Object.keys(man[pkgName].resolve.alias || {}).forEach(alias => {
            // resolves the relative path within a package to a full path
            rval.resolve.alias[alias] = path.resolve(
              NODE_MODULES_DIR,
              pkgName,
              man[pkgName].resolve.alias[alias]);
          });
        }
      });
